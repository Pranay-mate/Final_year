...
export const getInterests = () => async dispatch => {
    ...
    try{
        const res = await axios.get(`https://sleepy-reaches-43026.herokuapp.com/Final_year/interests/`+userId)
...
export const addInterests = userobj => async dispatch => {
    console.log(userobj)
    try{
        const res = await axios.post(`https://sleepy-reaches-43026.herokuapp.com/Final_year/interests`, userobj)
        dispatch( {
            type: ADD_INTEREST,
            payload: res.data
        })
...
export const deleteInterest = data => async dispatch => {
    ...
    const userId = user.result._id;
    try{
        const res = await axios.delete(`https://sleepy-reaches-43026.herokuapp.com/Final_year/interests/`+data)
        dispatch( {
...
export const updateInterest = data => async dispatch => {
    ...
    const userId = user.result._id;
    try{
        const res = await axios.put(`https://sleepy-reaches-43026.herokuapp.com/Final_year/interests/`+userId, data)
        dispatch( {
